...
app.get('/api/produtos', async (req, res) => {
  ...
  try {
    ...
    sql += ` ORDER BY nome ASC LIMIT $${params.length + 1} OFFSET $${
      params.length + 2
    }`;
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE produtos SET ${fields.join(
        ', ',
      )}, atualizadoem = NOW() WHERE id = $${fields.length + 1} RETURNING *`,
...
app.post('/api/produtos/valor-total', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT SUM(quantidade * valorunitario) as total FROM produtos',
    );
  ...
  try {
    ...
    await client.query(
      ...
      [
        uid(),
        produtoId,
        // 'saida',  <-- REMOVER ESTA LINHA. O SQL já tem 'saida' escrito.
        itemQuantidade, // Este será o $3
        `Entrega para: ${localObra}`, // Este será o $4
      ],
    );
...
app.delete('/api/movimentacoes/:id', async (req, res) => {
  ...
  try {
    ...
    if (mov.tipo === 'ajuste')
      throw new Error('Não é possível excluir ajuste.');
    ...
    res
      .status(200)
      .json({
        produtoAtualizado: toCamelCase({ ...produto, quantidade: novaQtd }),
      });
  } catch (err) {
    await client.query('ROLLBACK');
    res.status(500).json({ error: err.message });
...
app.post('/api/entregas', async (req, res) => {
  ...
  if (!produtoId || !itemQuantidade)
    return res
      .status(400)
      .json({ error: 'Produto e quantidade obrigatórios.' });
  ...
  try {
    ...
    if (!produto) throw new Error('Produto não encontrado.');
    if (produto.quantidade < itemQuantidade)
      throw new Error(
        `Estoque insuficiente (${produto.quantidade} disponível).`,
      );
    ...
    await client.query(
      `INSERT INTO movimentacoes (id, produtoid, tipo, quantidade, motivo, criadoem)
       VALUES ($1, $2, 'saida', $3, $4, NOW())`,
      [uid(), produtoId, 'saida', itemQuantidade, `Entrega para: ${localObra}`],
    );
...
// IMPORTANTE: Esta rota NÃO altera o estoque novamente, apenas dados cadastrais da entrega.
...
app.patch('/api/entregas/:id/status', async (req, res) => {
  try {
    await pool.query('UPDATE entregas SET status = $1 WHERE id = $2', [
      req.body.status,
      req.params.id,
    ]);
    res.json({ success: true });
